...
def valid_receipt(r):
    if not REQUIRED.issubset(r) or r.get("protocol")!="trigger/0.2": return False
    if any(not isinstance(r[k],str) or not r[k] for k in REQUIRED): return False
    try: issued=parse_time(r["issued_at"])
    except (ValueError,TypeError): return False
    if issued>NOW: return False
    if "expires_at" in r:
        try: expires=parse_time(r["expires_at"])
        except (ValueError,TypeError): return False
        if expires<=issued or expires<=NOW: return False
    return True
